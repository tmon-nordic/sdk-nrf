...
static void dect_shell_rssi_scan_cmd(const struct shell *shell, size_t argc, char **argv)
{
	...
	while ((opt = sys_getopt_long(argc, argv, "c:b:h", long_options_rssi_scan, &long_index)) !=
	       -1) {
		switch (opt) {
...
static void dect_shell_scan_cmd(const struct shell *shell, size_t argc, char **argv)
{
	...
	while ((opt = sys_getopt_long(argc, argv, "t:c:b:h", long_options_nw_scan, &long_index)) !=
	       -1) {
		switch (opt) {
		case 't': {
